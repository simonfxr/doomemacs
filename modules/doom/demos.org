...
This file adds demonstrations of Doom API for the [[doom-package:elisp-demos]]
package, installed by the [[doom-module::lang emacs-lisp]] module, which displays
these demos in documentation buffers (produced by help.el or helpful).
...
:END:
#+begin_src emacs-lisp :eval no
(define-key org-mode-map (kbd "C-j") (cmd! (newline) (indent-according-to-mode)))
#+end_src
...
inserts N newlines. We can use ~cmd!!~ to easily create keybinds that bake in the
prefix arg into the command call:

#+begin_src emacs-lisp :eval no
(define-key org-mode-map (kbd "C-j") (cmd!! #'newline 5))
#+end_src
...
(define-key evil-motion-state-map (kbd "TAB")
            (cmds! (and (bound-and-true-p yas-minor-mode)
                        (evil-visual-state-p)
